...
const SecondStep = async (req, res) => {
  ...
  if (!username || !image) {
    ...
    if (!user_id) {
      res
        .status(500)
        .json({ error: "Token not available on the database, Please relogin" });
    } else {
      if (image === 1) {
      ...
      try {
        const profileUpdate = await UsersProfile.updateOne(
          { user_id },
          { username, img }
        );
...
const UpdateDefaultCoin = async (req, res) => {
  ...
  if (!coin_name || !coin_image || !coin_bal) {
    ...
    if (!user_id) {
      ...
      if (coin_name === "BTC") {
        try {
          const Update = await defaultCoinDB.updateOne(
            { user_id },
            { coin_name, coin_image, coin_bal, wallet_address: coinWallet.BTC }
          );
        ...
        try {
          const Update = await defaultCoinDB.updateOne(
            { user_id },
            { coin_name, coin_image, coin_bal, wallet_address: coinWallet.ETH }
          );
          res.status(201).json(Update);
        } catch (err) {
          res.status(500).json({ message: err.message });
        ...
        try {
          const Update = await defaultCoinDB.updateOne(
            { user_id },
            { coin_name, coin_image, coin_bal, wallet_address: coinWallet.SOL }
          );
          res.status(201).json(Update);
        } catch (err) {
          res.status(500).json({ message: err.message });
        ...
        try {
          const Update = await defaultCoinDB.updateOne(
            { user_id },
            { coin_name, coin_image, coin_bal, wallet_address: coinWallet.USDT }
          );
          res.status(201).json(Update);
        } catch (err) {
          res.status(500).json({ message: err.message });
        ...
        try {
          const Update = await defaultCoinDB.updateOne(
            { user_id },
            { coin_name, coin_image, coin_bal, wallet_address: coinWallet.USDC }
          );
          res.status(201).json(Update);
        } catch (err) {
          res.status(500).json({ message: err.message });
        ...
        try {
          const Update = await defaultCoinDB.updateOne(
            { user_id },
            { coin_name, coin_image, coin_bal, wallet_address: coinWallet.BUSD }
          );
          res.status(201).json(Update);
        } catch (err) {
          res.status(500).json({ message: err.message });
        ...
        try {
          const Update = await defaultCoinDB.updateOne(
            { user_id },
            { coin_name, coin_image, coin_bal, wallet_address: coinWallet.NEXO }
          );
          res.status(201).json(Update);
        } catch (err) {
          res.status(500).json({ message: err.message });
        ...
        try {
          const Update = await defaultCoinDB.updateOne(
            { user_id },
            { coin_name, coin_image, coin_bal, wallet_address: coinWallet.BNB }
          );
          res.status(201).json(Update);
        } catch (err) {
          res.status(500).json({ message: err.message });
